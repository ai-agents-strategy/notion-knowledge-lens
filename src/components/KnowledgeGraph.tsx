...
export const KnowledgeGraph = ({ nodes, connections, showConnectionLabels }: KnowledgeGraphProps) => {
  ...
  useEffect(() => {
    ...
    const simulation = d3.forceSimulation(nodes as any)
      .force("link", d3.forceLink(connections).id((d: any) => d.id).distance(100))
    ...
    nodeGroups.append("circle")
      .attr("r", d => Math.max(20, d.size))
      .attr("fill", d => categoryColors[d.category as keyof typeof categoryColors] || "#6b7280")
      .attr("stroke", "#1e293b")
      .attr("stroke-width", 2)
    ...
    nodeGroups
      .on("mouseenter", (event, d) => {
        ...
        d3.select(event.currentTarget)
          ...
          .transition()
          .duration(200)
          .attr("r", (d: DatabaseNode) => Math.max(25, d.size + 5))
          .style("opacity", 1);
      })
      .on("mouseleave", (event, d) => {
        ...
        d3.select(event.currentTarget)
          ...
          .transition()
          .duration(200)
          .attr("r", (d: DatabaseNode) => Math.max(20, d.size))
          .style("opacity", 0.9);
      });
